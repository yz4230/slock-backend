CREATE TABLE IF NOT EXISTS users (id uuid PRIMARY KEY, "name" VARCHAR(64) NOT NULL, "password" VARCHAR(72) NOT NULL);
CREATE TABLE IF NOT EXISTS sessions (id uuid PRIMARY KEY, user_id uuid NOT NULL, token CHAR(16) NOT NULL, expires_at TIMESTAMP WITH TIME ZONE NOT NULL, CONSTRAINT fk_sessions_user_id__id FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE RESTRICT ON UPDATE RESTRICT);
CREATE TABLE IF NOT EXISTS channels (id uuid PRIMARY KEY, "name" VARCHAR(64) NOT NULL, description VARCHAR(1024) NOT NULL, is_direct BOOLEAN DEFAULT FALSE NOT NULL);
CREATE TABLE IF NOT EXISTS participates (user_id uuid, channel_id uuid, CONSTRAINT pk_participates PRIMARY KEY (user_id, channel_id), CONSTRAINT fk_participates_user_id__id FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE RESTRICT ON UPDATE RESTRICT, CONSTRAINT fk_participates_channel_id__id FOREIGN KEY (channel_id) REFERENCES channels(id) ON DELETE RESTRICT ON UPDATE RESTRICT);
CREATE TABLE IF NOT EXISTS messages (id uuid PRIMARY KEY, user_id uuid NOT NULL, channel_id uuid NOT NULL, thread_id uuid NULL, "content" VARCHAR(1024) NOT NULL);
CREATE TABLE IF NOT EXISTS attachments (id uuid PRIMARY KEY, message_id uuid NOT NULL, filename VARCHAR(128) NOT NULL, "path" TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS reactions (id uuid PRIMARY KEY, message_id uuid NOT NULL, user_id uuid NOT NULL, emoji VARCHAR(64) NOT NULL);
